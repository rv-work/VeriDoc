...
const Login: React.FC<LoginProps> = ({ onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      const response = await fetch('https://veridoc.onrender.com/api/auth/login', {
        method: 'POST',
        headers: {
    ...
    <div className="bg-white rounded-2xl shadow-2xl  w-full max-h-[90vh] overflow-y-auto">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-4">
        ...
        <button
          type="submit"
          disabled={isLoading}
          className={`w-full cursor-pointer py-3 px-4 rounded-xl font-semibold text-white transition-all duration-200 ${isLoading
